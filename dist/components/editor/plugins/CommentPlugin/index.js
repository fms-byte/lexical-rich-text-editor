...
exports.INSERT_INLINE_COMMAND = void 0;
...
const LexicalEditorRefPlugin_1 = require("@lexical/react/LexicalEditorRefPlugin");
...
function PlainTextEditor({ className, autoFocus, onEscape, onChange, editorRef, placeholder = 'Type a comment...', }) {
    ...
    return (React.createElement(LexicalComposer_1.LexicalComposer, { initialConfig: initialConfig },
        React.createElement("div", { className: "CommentPlugin_CommentInputBox_EditorContainer" },
            ...
            autoFocus !== false && React.createElement(LexicalAutoFocusPlugin_1.AutoFocusPlugin, null),
            ...
            editorRef !== undefined && React.createElement(LexicalEditorRefPlugin_1.EditorRefPlugin, { editorRef: editorRef }))));
}
function useOnChange(setContent, setCanSubmit) {
    return (0, react_1.useCallback)((editorState, _editor) => {
        editorState.read(() => {
...
function CommentInputBox({ editor, cancelAddComment, submitAddComment, }) {
    ...
    const updateLocation = (0, react_1.useCallback)(() => {
                ...
                if (range !== null && boxElem !== null) {
                    ...
                    for (let i = 0; i < selectionRectsLength; i++) {
                        ...
                        const style = `position:absolute;top:${selectionRect.top +
                            (window.pageYOffset || document.documentElement.scrollTop)}px;left:${selectionRect.left}px;height:${selectionRect.height}px;width:${selectionRect.width}px;background-color:rgba(${color}, 0.3);pointer-events:none;z-index:5;`;
...
function ShowDeleteCommentOrThreadDialog({ commentOrThread, deleteCommentOrThread, onClose, thread = undefined, }) {
    return (React.createElement(React.Fragment, null,
...
function CommentsPanelListComment({ comment, deleteComment, thread, rtf, }) {
    ...
    return (React.createElement("li", { className: "CommentPlugin_CommentsPanel_List_Comment" },
        ...
        !comment.deleted && (React.createElement(React.Fragment, null,
            React.createElement(Button_1.default, { onClick: () => {
                    showModal('Delete Comment', (onClose) => (React.createElement(ShowDeleteCommentOrThreadDialog, { commentOrThread: comment, deleteCommentOrThread: deleteComment, thread: thread, onClose: onClose })));
...
function CommentsPanelList({ activeIDs, comments, deleteCommentOrThread, listRef, submitAddComment, markNodeMap, }) {
    ...
    const [modal, showModal] = (0, useModal_1.default)();
    const rtf = (0, react_1.useMemo)(() => new Intl.RelativeTimeFormat('en', {
        ...
        if (commentOrThread.type === 'thread') {
            ...
            React.createElement("li", { key: id, onClick: handleClickThread, className: `CommentPlugin_CommentsPanel_List_Thread ${markNodeMap.has(id) ? 'interactive' : ''} ${activeIDs.indexOf(id) === -1 ? '' : 'active'}` },
                React.createElement("div", { className: "CommentPlugin_CommentsPanel_List_Thread_QuoteBox" },
                    ...
                    React.createElement(Button_1.default, { onClick: () => {
                            showModal('Delete Thread', (onClose) => (React.createElement(ShowDeleteCommentOrThreadDialog, { commentOrThread: commentOrThread, deleteCommentOrThread: deleteCommentOrThread, onClose: onClose })));
                        }, className: "CommentPlugin_CommentsPanel_List_DeleteButton" },
                        React.createElement("i", { className: "delete" })),
                    modal),
...
function useCollabAuthorName() {
    const collabContext = (0, LexicalCollaborationContext_1.useCollaborationContext)();
    const { yjsDocMap, name } = collabContext;
    return yjsDocMap.has('comments') ? name : 'Playground User';
}
function CommentPlugin({ providerFactory, }) {
    const collabContext = (0, LexicalCollaborationContext_1.useCollaborationContext)();
    const [editor] = (0, LexicalComposerContext_1.useLexicalComposerContext)();
    const commentStore = (0, react_1.useMemo)(() => new commenting_1.CommentStore(editor), [editor]);
    const comments = (0, commenting_1.useCommentStore)(commentStore);
    const markNodeMap = (0, react_1.useMemo)(() => {
        return new Map();
    }, []);
    const [activeAnchorKey, setActiveAnchorKey] = (0, react_1.useState)();
    const [activeIDs, setActiveIDs] = (0, react_1.useState)([]);
    const [showCommentInput, setShowCommentInput] = (0, react_1.useState)(false);
    ...
    const cancelAddComment = (0, react_1.useCallback)(() => {
        editor.update(() => {
            const selection = (0, lexical_1.$getSelection)();
            // Restore selection
            if (selection !== null) {
    ...
    const deleteCommentOrThread = (0, react_1.useCallback)((comment, thread) => {
    ...
    const submitAddComment = (0, react_1.useCallback)((commentOrThread, isInlineComment, thread, selection) => {
    ...
    return (React.createElement(React.Fragment, null,
        ...
        (0, react_dom_1.createPortal)(React.createElement(Button_1.default, { className: `CommentPlugin_ShowCommentsButton ${showComments ? 'active' : ''}`, onClick: () => setShowComments(!showComments), title: showComments ? 'Hide Comments' : 'Show Comments' },
            React.createElement("i", { className: "comments" })), document.body),
        showComments &&
